...
void ddtrace_execute_internal(zend_execute_data *execute_data, zend_fcall_info *fci, int return_value_used TSRMLS_DC) {
    ...
    if (dispatch && _dd_should_trace_dispatch(dispatch TSRMLS_CC) && (dispatch->options & DDTRACE_DISPATCH_POSTHOOK)) {
        dispatch->busy = 1;
        ...
        zend_op *opline = EX(prev_execute_data) ? EX(prev_execute_data)->opline : NULL;  // ?
...
void ddtrace_execute_ex(zend_execute_data *execute_data TSRMLS_DC) {
    ...
    if (dispatch && _dd_should_trace_dispatch(dispatch TSRMLS_CC) && (dispatch->options & DDTRACE_DISPATCH_POSTHOOK)) {
        ...
        _dd_execute_end_span(execute_data->prev_execute_data, span, *EG(return_value_ptr_ptr), opline TSRMLS_CC);

        if (EG(return_value_ptr_ptr) == &retval && !EG(exception)) {
